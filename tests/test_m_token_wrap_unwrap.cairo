...
func test_wrap_token{
    ...
    let (erc20_bal_new : Uint256) = IERC20.balanceOf(contract_address=erc20_address, account=contract_address)
    local erc20_bal_new : Uint256 = erc20_bal_new
    %{print(f"[After UNwrap]m_token's erc20_token_bal.low: {ids.erc20_bal_new.low}")%}
    let (is_balance_eq) = uint256_eq(Uint256(3,0), erc20_bal_new)
    assert is_balance_eq = 1
